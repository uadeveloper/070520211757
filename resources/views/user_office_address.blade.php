                        ...
                        <div class="uofb cf">
                            <div class="l_col adrs">
                                ...
                                <form action="/address" method="POST">
                                    ...
                                    <div class="field">
                                        <label>Name *</label>
                                        <input type="text" name="name" value="{{ old('name') }}" palceholder="" class="vl_empty @error('name') is-invalid @enderror" />
                                    </div>
                                    <div class="field">
                                        <label>Your city *</label>
                                        <select name="city" class="vl_empty @error('city') is-invalid @enderror">
                                            <option class="plh"></option>
                                            <option @if (old('city') == 'Kharkiv') selected="selected" @endif value="Kharkiv">Kharkiv</option>
                                            <option @if (old('city') == 'Kiyv') selected="selected" @endif value="Kiyv">Kiyv</option>
                                        </select>
                                    </div>
                                    <div class="field">
                                        <label>Your area *</label>
                                        <select name="area" class="@error('area') is-invalid @enderror">
                                            <option class="plh"></option>
                                            <option @if (old('area') == 'Area 1') selected="selected" @endif>Area 1</option>
                                            <option @if (old('area') == 'Area 2') selected="selected" @endif>Area 2</option>
                                        </select>
                                    </div>
                                    <div class="field">
                                        <label>Street</label>
                                        <input type="text" name="street" value="{{ old('street') }}" palceholder="" class="vl_empty @error('street') is-invalid @enderror"/>
                                    </div>
                                    <div class="field">
                                        <label>House # </label>
                                ...
                                <div class="uo_adr_list">
                                    @foreach ($addresses as $address)
                                        <div class="item">
                                            <h3>{!! $address->name !!}</h3>
                                            <p>
                                                {!! $address->city !!}@if ($address->city),@endif
                                                {!! $address->area !!}@if ($address->area),@endif
                                                {!! $address->street !!}@if ($address->street && $address->house),@endif
                                                {!! $address->house !!}@if ($address->house && $address->additional_information),@endif
                                                {!! $address->additional_information !!}
                                                <a href="https://www.google.com/maps/search/?api=1&query={!! $address->city !!}, {!! $address->area !!}, {!! $address->street !!} {!! $address->house !!}" target="_blank">Open on google maps</a>
                                            </p>
                                            <div class="actbox">
